...
export default function CardDesafios(props: DesafioList) {
    if (!props.desafios || props.desafios.length === 0) {
        return (
            <div className="mx-auto max-w-[520px] text-center px-8 py-12 border border-dashed border-bege/30 rounded-card">
            ...
            <div className="flex flex-wrap justify-center w-full gap-5">
                {
                    props.desafios.map((desafioObj, idx) => (
                        <article key={idx} className="flex flex-col justify-evenly px-10 py-6 w-[315px] [@media(min-width:420px)]:w-[380px] min-h-[200px] bg-laranja rounded-[1.2rem] text-center text-verdescuro2 transition-all duration-150 hover:ring-4 hover:ring-azulelegante">
                            <h2 className="text-xl font-eloquent font-extrabold text-verdescuro2">{desafioObj.desafio}</h2>
                            <p className="flex items-center flex-col font-roboto text-base">
                                <a href={desafioObj.enunciado} target="_blank" rel="noreferrer" className="mb-[5px] mt-[5px] text-lg font-subtitulo font-bold text-verdescuro2 w-fit transition-all duration-100 hover:scale-[1.1] hover:text-white">Enunciado</a>
